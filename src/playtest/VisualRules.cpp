...
ScreenPoint VisualRules::ProjectPointClamped(const PlaytestState& state, int screenWidth, int screenHeight, float worldX, float worldY,
    ...
    z2 = std::max(z2, std::max(clampDepth, rules.minDepth));

    const int sx = static_cast<int>(screenWidth * 0.5F + (x1 / z2) * rules.focalLength);
    const int sy2 = static_cast<int>(screenHeight * 0.5F - (y2 / z2) * rules.focalLength);
    const bool visible = sx >= -200 && sx <= screenWidth + 200 && sy2 >= -200 && sy2 <= screenHeight + 200;
    return ScreenPoint{sx, sy2, visible};
}
